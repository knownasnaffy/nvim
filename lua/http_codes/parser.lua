...
function M.find_section15(root, html)
  return M.find_element_with_id_recursive(root, html, 'section', 'section-15')
end
...
function M.find_element_with_id_recursive(node, html, tag, id)
  if node:type() == 'element' then
    ...
    if node_tag == tag and node_id == id then
      return node
    end
  end
...
function M.find_section_name_link(h4, html)
  return M.find_element_with_class(h4, html, 'a', 'section-name')
end
...
function M.get_text_content(node, html)
  ...
  for child in node:iter_children() do
    if child:type() == 'text' then
      text = text .. html:sub(child:start() + 1, child:end_())
    end
  end
  return text
end
...
function M.get_tag_name(element, html)
  ...
  if start_tag and start_tag:type() == 'start_tag' then
    ...
    for i = 0, start_tag:child_count() - 1 do
      ...
      if child and child:type() == 'tag_name' then
        return html:sub(child:start() + 1, child:end_())
      end
    end
  end
  return nil
        ...
        if name == attr_name then
          ...
          if value_node then
            if value_node:type() == 'quoted_attribute_value' then
              ...
              if inner_value and inner_value:type() == 'attribute_value' then
                return html:sub(inner_value:start() + 1, inner_value:end_())
              end
