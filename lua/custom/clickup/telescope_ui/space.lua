...
local select_list = require 'custom.clickup.telescope_ui.list'

local function select_space(workspace_id)
  local token = get_clickup_token()
  local url = 'https://api.clickup.com/api/v2/team/' .. workspace_id .. '/space'
  ...
  if not data.spaces or #data.spaces == 0 then
  ...
  for _, space in ipairs(data.spaces) do
    table.insert(choices, { display = space.name, id = space.id })
  end
